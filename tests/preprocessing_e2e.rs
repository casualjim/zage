...
use zage::model::pretrained_embedder::PretrainedEmbedder;
...
fn create_complex_command() -> Invocation {
    Invocation {
        command: "find . -type f -name \"*.rs\" | xargs grep -l \"fn main\" | sort -r | head -n 5".to_string(),
        shellname: "bash".to_string(),
        working_directory: Some("/home/user/project".to_string()),
        hostname: Some("laptop".to_string()),
        username: Some("user".to_string()),
...
fn test_embedding_various_commands() -> Result<()> {
    ...
    let complex_cmd = embedder.embed(
        "find . -type f -name \"*.rs\" | xargs grep \"fn main\" | sort | head -n 5"
    )?;
    
    // Verify embeddings have consistent dimensions
    assert_eq!(basic_cmd.len(), git_cmd.len(), "Embeddings should have consistent dimensions");
    assert_eq!(basic_cmd.len(), complex_cmd.len(), "Embeddings should have consistent dimensions");
    ...
    assert!(git_distance < unrelated_distance, 
...
fn test_context_feature_extraction() -> Result<()> {
    ...
    let project_features = context_features.encode_working_directory(
        &invocations[0].working_directory.clone().unwrap()
    )?;
    let docker_features = context_features.encode_working_directory(
        &invocations[5].working_directory.clone().unwrap()
    )?;
    
    // Different directories should produce different embeddings
    assert_ne!(project_features, docker_features, 
        "Different directories should produce different embeddings");
    ...
    assert_ne!(success_features, error_features, 
        "Different exit statuses should produce different features");
    assert_eq!(success_features.len(), 2, "Exit status features should have 2 dimensions");
    
    // Test time features
    let morning_time = 1620000000; // Some time in the morning
    let evening_time = 1620050000; // Some time in the evening
    
    let morning_features = context_features.encode_time_features(Some(morning_time))?;
    let evening_features = context_features.encode_time_features(Some(evening_time))?;
    
    assert_ne!(morning_features, evening_features, 
        "Different times should produce different features");
    assert_eq!(morning_features.len(), 4, "Time features should have 4 dimensions");
    
    // Test all features together
    let all_features = context_features.encode_all_features(&invocations[0])?;
    
    // Should have substantial number of features
    assert!(all_features.len() > 10, "Combined features should have substantial dimensions");
...
fn test_feature_integration() -> Result<()> {
    ...
    assert_eq!(git_features.len(), docker_features.len(), 
        "Integrated features should have consistent dimensions");
    assert_eq!(git_features.len(), file_features.len(), 
        "Integrated features should have consistent dimensions");
    
    // Normalized features should have unit length
    assert_almost_eq(vector_norm(&git_features), 1.0, 1e-5, 
        "Normalized features should have unit length");
    ...
    // Tensor should have correct dimensions
    assert_eq!(tensor.dims(), &[3, git_features.len()], 
...
fn test_training_dataset_creation() -> Result<()> {
    ...
    assert_eq!(dataset.len(), expected_examples, 
    ...
    assert_eq!(train.len(), expected_train_size, 
        "Training set should have expected size");
    assert_eq!(val.len(), expected_val_size, 
    ...
    if !batches.is_empty() {
        ...
        assert_eq!(first_batch.input.dims()[0], 
        ...
        assert_eq!(first_batch.target.dims().len(), 2, 
...
fn test_end_to_end_preprocessing() -> Result<()> {
    ...
    let embedder = Arc::new(PretrainedEmbedder::new(device.clone())?);
    let context_features = Arc::new(ContextualFeatures::new(device.clone())?);
    let _integrator = FeatureIntegrator::new(
        embedder.clone(),
        context_features.clone(),
        device.clone(),
    )?;
...
fn benchmark_preprocessing_pipeline() -> Result<()> {
    ...
    println!("  Command Embedding:        {:?} avg/command", embedding_avg);
    println!("  Context Feature Extract:  {:?} avg/command", context_avg);
    println!("  Feature Integration:      {:?} avg/command", integration_avg);
