...
use zage::Result;
use zage::ZageError;
use zage::socket_server::MessageType;
use zage::socket_server::encoder::{LengthDelimitedDecoder, LengthDelimitedEncoder};
...
fn send_embed_request(stream: &mut UnixStream, text: &str) -> Result<Vec<f32>> {
  ...
  stream.write_all(&[MessageType::EmbedRequest.to_byte()])?;

  // Write the request length
  let len_bytes = (encoded.len() as u32).to_le_bytes();
  stream.write_all(&len_bytes)?;

  // Write the RLE-encoded request
